...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="cul-content-layout">';
        $result .= '<div class="cul-content-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="cul-layout-cell' . ('cul-block' == $style ? ' cul-layout-sidebar-bg' : '')
     ...
     *
     * The componentWrapper method gets the content of the 'component' buffer and searches for the '<div class="cul-post">' string in it.
     * Then it wraps content of the buffer with cul-post.
     */
    public function componentWrapper()
    {
        ...
        if ('com_content' == $option && ('featured' == $view || 'article' == $view || ('category' == $view && 'blog' == $layout)))
            return;
        if (false === strpos($content, '<div class="cul-post'))
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'cul-main').addClass('cul-postcontent');
            }
        });
    }
});
